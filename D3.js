...
let crewMassList = []
for (let i = 0; i < starWarsCharacters.length; i++) {
  crewMassList.push(starWarsCharacters[i].mass)
}
console.log(crewMassList)

let crewMass = 0
let j = 0
while (j < crewMassList.length) {
  crewMass += crewMassList[j]
  j++
}
console.log("The total of the crew members' masses is", crewMass)
...
for (let i = 0; i < starWarsCharacters.length; i++) {
  if (starWarsCharacters[i].gender === 'robot') {
    console.log(
      starWarsCharacters[i].name + ' - ' + starWarsCharacters[i].gender
    )
  }
}
